...
export function CreateAccountButtons({ accounts, onAccountCreated }: CreateAccountButtonsProps) {
  ...
  const hasAllAccountTypes = ['epargne', 'courant', 'béni'].every(type =>
    accounts.some(account => account.name === getAccountTypeName(type as AccountType))
  );
    ...
    <div className="bg-white rounded-xl border p-6 shadow-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {(['epargne', 'courant', 'béni'] as AccountType[]).map((type) => {
          ...
          return (
            <Popover key={type} open={isOpen && selectedType === type} onOpenChange={(open) => {
              setIsOpen(open);
                ...
                <div className="grid gap-4">
                  <div className="space-y-2">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Veuillez effectuer un dépôt initial pour activer votre compte.
                    </p>
                  </div>
                  <CreateAccountForm
                    type={type}
